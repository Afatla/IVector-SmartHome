...
class IVector():
    ...
    def save_ivector_to_file(self, ivector, path):
    ...
    def get_score_from_ivectors(self, path, plda_model_dir="../models/backend"):
        ...
        enroll_ivec = self.load_vectors_into_ndarray(np.asarray(files), prefix="", suffix="",
                                                        dtype=np.float32)

        test_ivec = self.load_vectors_into_ndarray(np.asarray(files), prefix="", suffix="",
                                                     dtype=np.float32)
        ...
        s = self.bilinear_plda(Lambda, Gamma, c, k, enroll_ivec, test_ivec)
        print(s)
    ...
    def save_ivectors(self, directory_waves, directory_ivectors):
        for file in os.listdir(directory_waves):
            ivector = self.process_wav(directory_waves+"/"+file,
                                       mode="ivector")
